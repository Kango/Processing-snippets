...
void stateFileManagerFunction() {
  ...
  textTab ("Folder does not exist:\n"
    + pathGlobal
    + "\nPlease change folder. "
    + "Please note that \nthe folder itself (not only its subfolders) must contain mp3s. You can set the \nfolders in the source code.", 20, 20 );
  ...
  if (folderDoesExist) {
    ...
    fill(255); 
    text("Use Mouse Wheel to scroll.", 12, width-22);
  } else {
